...
const ProjectCard = ({ project }) => {
  ...
  return (
    <div className="bg-raisin-black-400 rounded-lg shadow-md p-5 hover:shadow-lg hover:scale-105 transition-transform duration-300 ease-in-out">
      ...
      <h2 className="text-xl font-semibold text-buff-500 mb-2">{project.name}</h2>
      <p className="text-gray-400 mb-2">{truncate(project.shortDescription, 80)}</p>
      <div className="flex flex-wrap gap-2 mt-3">
        {project.students.map((student, index) => (
          <span key={index} className="bg-yellow-green-500 text-raisin-black-100 text-sm px-2 py-1 rounded-full">
            {student}
          </span>
        ))}
      </div>
